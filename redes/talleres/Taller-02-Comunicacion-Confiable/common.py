...
# Stop and wait extiende el formato de header básico agregándole un campo
# "seq_num" de 8 bits cuyos únicos valores posibles son 0 o 1 (bit alternante,
# ver Peterson & Davie 2.5). El acknowledge deja de ser un frame vacío: ahora
# lleva el seq_num al que se le está haciendo ack.

STOPANDWAIT_DATA_HEADER_SIZE = 3 + 1
STOPANDWAIT_DATA_FRAME_SIZE = STOPANDWAIT_DATA_HEADER_SIZE + CHUNK_SIZE
STOPANDWAIT_ACK_FRAME_SIZE = 1

def encode_stopandwait_data_frame(is_last, seq_num, data):
    assert seq_num in (0, 1)
    return struct.pack('!bhb', bool(is_last), len(data), seq_num) + pad_to_chunk_size(data)

def decode_stopandwait_data_frame(frame):
    (is_last, length, seq_num) = struct.unpack('!bhb', frame[:STOPANDWAIT_DATA_HEADER_SIZE])
    assert is_last in (0, 1)
    assert seq_num in (0, 1)
    padded_data = frame[STOPANDWAIT_DATA_HEADER_SIZE:]
    data = padded_data[:length]
    return bool(is_last), seq_num, data

def encode_stopandwait_ack_frame(seq_num):
    assert seq_num in (0, 1)
    return struct.pack('!b', seq_num)

def decode_stopandwait_ack_frame(frame):
    (seq_num,) = struct.unpack('!b', frame)
    return seq_num
